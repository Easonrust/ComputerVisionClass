...
def laplacian_of_gaussian_filter(sigma):
    ...
    x, y = np.meshgrid(
        np.arange(-half_size, half_size + 1), np.arange(-half_size, half_size + 1)
    )
...
def max_supression(scale_space, sigma, threshold_factor, level):
    ...
    def check(l):
        return all(
            scale_space[i + dx, j + dy, l] < scale_space[i, j, k]
            for dx, dy in index
    ...
    for k in range(0, level):
        ...
        for i in range(mask[k] + 1, size[0] - mask[k] - 1):
            for j in range(mask[k] + 1, size[1] - mask[k] - 1):
                ...
                if k - 1 >= 0:
                    l_max = (
                        check(k - 1) and scale_space[i, j, k - 1] < scale_space[i, j, k]
                    )
                if k + 1 < level:
                    u_max = (
                        check(k + 1) and scale_space[i, j, k + 1] < scale_space[i, j, k]
...
def main():
    ...
    cv2.imshow(
        "Blob Threshold={},Initial Sigma={}, {} circles".format(
            threshold_factor, initial_sigma, no_of_blobs
        ),
        image,
    )
    cv2.waitKey(0)
